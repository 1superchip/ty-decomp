...
void GXSetTexCoordScaleManually(GXTexCoordID coord, u8 enable, u16 ss, u16 ts) {
  ...
  if (enable != 0) {
    SET_REG_FIELD(gx->suTs0[coord], 16, 0, (u16)(ss - 1));
    SET_REG_FIELD(gx->suTs1[coord], 16, 0, (u16)(ts - 1));
...
void GXSetTexCoordBias(GXTexCoordID coord, u8 s_enable, u8 t_enable) {
  SET_REG_FIELD(gx->suTs0[coord], 1, 16, s_enable);
  SET_REG_FIELD(gx->suTs1[coord], 1, 16, t_enable);
...
static void __SetSURegs(u32 tmap, u32 tcoord) {
  ...
  field = w;
  SET_REG_FIELD(gx->suTs0[tcoord], 16, 0, field);
  field = h;
  SET_REG_FIELD(gx->suTs1[tcoord], 16, 0, field);
  s_bias = GET_REG_FIELD(gx->tMode0[tmap], 2, 0) == 1;
  t_bias = GET_REG_FIELD(gx->tMode0[tmap], 2, 2) == 1;
  field = s_bias;
  SET_REG_FIELD(gx->suTs0[tcoord], 1, 16, field);
  field = t_bias;
  SET_REG_FIELD(gx->suTs1[tcoord], 1, 16, field);
