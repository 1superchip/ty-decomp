...
// these templates should be weak but they aren't generating in the correct order
// "__declspec(weak)" forces the symbol to be weak like it should be
template <>
__declspec(weak) void LoadDescriptors(KromeIni* pIni, char* name, PlatformDesc* pDesc) {
    ...
    while (pLine != NULL && (pLine->section != NULL || pLine->pFieldName != NULL || pLine->comment != NULL)) {
        ...
        if (pLine->pFieldName != NULL && pLine->AsString(0, &pString) != false) {
            p = (GameObjDesc*)Heap_MemAlloc(sizeof(PlatformDesc));
            memset(p, 0, sizeof(PlatformDesc));
            new ((void*)p) PlatformDesc;
            static_cast<PlatformDesc*>(p)->Init(pDesc->pModule, pString, pLine->pFieldName, pDesc->searchMask, pDesc->flags);
...
template <>
__declspec(weak) void PtrList<GameObject>::Destroy(GameObject* pTemplate) {
    GameObject** ptrs = pPointers;
    while (*ptrs != NULL) {
        if (*ptrs == pTemplate) {
            *ptrs = *pPointers++;
            return;
